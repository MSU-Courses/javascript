...
- [Объекты](#объекты)
  ...
  - [Что такое объект в JavaScript?](#что-такое-объект-в-javascript)
  - [Для чего используются объекты?](#для-чего-используются-объекты)
  - [Объекты в JavaScript](#объекты-в-javascript)
    - [Определение](#определение)
    - [Создание объекта](#создание-объекта)
    - [Свойства объекта](#свойства-объекта)
      - [Определение свойств объекта](#определение-свойств-объекта)
      - [Ключи](#ключи)
      - [Типы данных свойств](#типы-данных-свойств)
      - [Получение свойств](#получение-свойств)
      - [Добавление свойств](#добавление-свойств)
      - [Удаление свойств](#удаление-свойств)
      - [Вычисляемые свойства](#вычисляемые-свойства)
      - [Свойство из переменной](#свойство-из-переменной)
  ...
  - [Объект - ссылочный тип данных](#объект---ссылочный-тип-данных)
    - [Особенности ссылочного типа данных](#особенности-ссылочного-типа-данных)
    - [Клонирование объектов](#клонирование-объектов)
      - [Использование `Object.assign()`](#использование-objectassign)
      - [Использование spread-оператора](#использование-spread-оператора)
  - [Деструктуризация объектов](#деструктуризация-объектов)
    - [Синтаксис](#синтаксис)
    - [Переименование свойства](#переименование-свойства)
  ...
  - [null](#null)
    ...
    - [Разница между `null` и `undefined`](#разница-между-null-и-undefined)
    - [Пример использования `null`](#пример-использования-null)
  - [Коллекции `Map` и `Set`](#коллекции-map-и-set)
    - [Коллекция `Map`](#коллекция-map)
    - [Коллекция `Set`](#коллекция-set)
  - [Встроенные объекты и методы примитивов](#встроенные-объекты-и-методы-примитивов)
    - [Объект `Math`](#объект-math)
      - [Основные константы Math](#основные-константы-math)
      - [Основные методы Math](#основные-методы-math)
    - [Объект `Date`](#объект-date)
      - [Создание объектов Date](#создание-объектов-date)
      - [Основные методы `Date`](#основные-методы-date)
    - [Методы примитивов](#методы-примитивов)
  - [Массивы - это объекты](#массивы---это-объекты)
...
_Объекты являются примером ссылочного типа данных_. Это означает, что они хранятся и копируются по ссылке, а не по значению.

Простыми словами, _ссылочный тип данных_ - это ситуация, когда переменная не хранит само значение, а лишь ссылается на область памяти, где это значение находится.
...
Когда переменная содержит объект, она не хранит сам объект, а запоминает ссылку на место в памяти, где этот объект находится. Можно представить, что переменная хранит не сам объект, а «_записку с адресом_», по которому объект лежит в памяти компьютера.
...
Этот момент часто вызывает трудности на начальном этапе. Если сейчас он кажется не до конца понятным - это нормально. Главное - запомнить ключевую идею: _объекты при присваивании не копируются, а передаются по ссылке_.
...
В контексте объектов spread-оператор делает следующее: он берёт все свойства объекта и _«разворачивает»_ их в новый объект. Проще говоря, spread-оператор облегчает создание новых объектов, позволяя объединять свойства из нескольких объектов в один.
...
На практике часто возникает ситуация, когда хочется использовать другое имя переменной, отличное от имени свойства объекта. Например, вместо переменной `brand` использовать более короткое имя `b`. Для этого в деструктуризации применяется _переименование через двоеточие_.
...
На практике часто встречаются объекты со вложенной структурой, где один объект содержит другие объекты или массивы. В таких случаях можно применять _вложенную деструктуризацию_.
...
В JavaScript `null` не является ссылкой на несуществующий объект и не является нулевым указателем, как в некоторых других языках программирования. Значение `null` буквально означает: «_ничего_» или «_значение отсутствует намеренно_».
...
Другими словами, null — это специальное значение, которое мы используем, чтобы явно показать: «_Здесь должно быть значение (обычно объект), но сейчас его нет_».
...
- _Любые типы ключей_. В отличие от объектов, где ключами могут быть только строки или символы, Map позволяет использовать в качестве ключей любые типы данных, включая объекты, функции или примитивные значения.
- _Порядок итерации_. Элементы в Map сохраняются в том порядке, в котором они были добавлены. _В объектах порядок свойств может быть неопределенным и зависеть от реализации JavaScript-движка_.
- _Легкость очистки_. Для удаления всех элементов из Map можно использовать метод `clear()`, в то время как в объектах для этого придется перебирать все свойства и удалять их по отдельности.
- _Легкость определения размера_. Свойство `size` в Map предоставляет простой способ получения количества элементов, в то время как в объектах приходится использовать цикл или другие методы для подсчета свойств.
- _Управление памятью_. Map лучше подходит для кэширования больших объемов данных или временных значений, так как он может быть очищен с помощью метода `clear()`, что упрощает управление памятью.
...
> [!NOTE]
>
> Данная тема является темой для самостоятельного изучения.

Помимо пользовательских объектов, которые мы научились создавать самостоятельно, в JavaScript существуют встроенные объекты, предоставляемые языком по умолчанию.

К таким встроенным объектам относятся, например: `Math`, `Object`, `Array`, `Date`, `RegExp` и другие. Они решают типовые задачи и позволяют не писать заново то, что уже давно реализовано и оптимизировано.

Например:

- объект `Math` содержит математические функции и константы;
- объект `Array` предоставляет методы для работы с массивами;
- объект `Date` позволяет работать с датами и временем;
- объект `Object` содержит методы для работы с объектами.

Рассмотрим несколько примеров использования встроенных объектов.

### Объект `Math`

Объект Math предоставляет набор математических констант и функций.

#### Основные константы Math

- `Math.PI` - число π (пи), примерно `3.14159`
...
- `Math.floor(x)` - округляет число вниз
- `Math.ceil(x)` - округляет число вверх
...
_Минимум и максимум_

- `Math.min(...args)` - возвращает наименьшее из аргументов
- `Math.max(...args)` - возвращает наибольшее из аргументов

```js
Math.min(3, 5, 1); // 1
Math.max(3, 5, 1, -1, 0); // 5
```

_Случайные числа_

- `Math.random()` - возвращает псевдослучайное число от 0 (включительно) до 1 (исключительно)

```js
Math.random(); // Например, 0.123456789
```

Случайное целое число в диапазоне `[a, b]` считается по следующей формуле: `R = Math.floor(Math.random() * (b - a + 1)) + a`.
...
getRandomInt(1, 10); // Например, 7
```

### Объект `Date`

Объект `Date` используется для работы с датой и временем. В отличие от `Math`, `Date` создаётся через конструктор.

#### Создание объектов Date

_Текущая дата и время_:
...
_Создание даты вручную_:

```js
const specificDate = new Date(2020, 0, 1); // 1
console.log(specificDate); // Wed Jan 01 2020
```

#### Основные методы `Date`

_Получение компонентов даты_
...
_Получение времени в миллисекундах_
...
- `setMonth(month)` - установить месяц (0–11)
...
date.setDate(25);
console.log(date); // Sat Dec 25 2021
```

### Методы примитивов

В прошлых темах мы рассмотрели 6 примитивных типов данных: `string`, `number`, `boolean`, `null`, `undefined` и `symbol`.
...
Странно, правда? Мы же знаем, что `str` - это строка, а строки являются примитивами. Тем не менее мы вызываем у строки методы, которые обычно ассоциируются с объектами. То же самое и с числом `num`: это примитив, но у него есть методы. Как такое возможно?

Когда JavaScript проектировался, возник следующий парадокс:

1. С примитивами (строками, числами и т.д.) хочется выполнять множество операций, и было бы удобно вызывать их через методы, а не через глобальные функции, например `toUpperCase(str)` или `toFixed(num)`.
2. При этом примитивы должны оставаться максимально лёгкими и быстрыми.

Выбранное решение выглядит немного необычно, но оно работает:

1. Примитивы остаются примитивами. Строка _"Hello"_ - это просто строка, число _42_ - это просто число.
2. Когда мы пытаемся вызвать метод у примитива (например, `str.toUpperCase()`), JavaScript временно создаёт специальный объект-обёртку, предоставляет нужный метод, а затем сразу удаляет этот объект.

Каждый примитив имеет свой соответствующий объект-обёртку:
...
Именно поэтому у разных примитивов доступен разный набор методов.

Например, при вызове `str.toUpperCase()` происходит следующее:

1. Создаётся временный объект `String`, который оборачивает значение `str`. Этот объект имеет все методы, определённые для строк.
2. У этого временного объекта вызывается метод `toUpperCase()`.
3. Результат метода возвращается, а временный объект удаляется.

Можно создавать такие объекты-обёртки вручную, но это не рекомендуется, так как они занимают больше памяти и могут приводить к путанице:

```js
const strObj = new String('Hello'); // не рекомендуется
```

Современные JavaScript-движки сильно оптимизируют этот процесс. На практике они могут даже не создавать реальный объект, но при этом обязаны вести себя так, как будто он был создан, чтобы соответствовать спецификации языка.

Главное, что стоит запомнить: _примитивы остаются примитивами, а методы у них работают благодаря временным объектам-обёрткам, которые JavaScript создаёт автоматически_.

## Массивы - это объекты

> [!NOTE]
>
> Эта глава является дополнительной. Её содержание не включается в экзаменационные материалы и контрольные вопросы и предназначено для более глубокого понимания языка.

В прошлых главах мы уже говорили, что массивы в JavaScript - это _особый тип объектов_. Теперь у вас достаточно контекста, чтобы понять, что именно это означает на практике.

С точки зрения JavaScript, массив — это объект, у которого:

- ключами являются числовые индексы (`0`, `1`, `2`, …),
- значениями - элементы массива.

Именно поэтому доступ к элементу массива через квадратные скобки:

```js
arr[0];
```

на самом деле является обычным доступом к свойству объекта и эквивалентен записи:

```js
obj[key];
```

где массив выступает в роли объекта, а индекс — в роли ключа.

Хотя массивы являются объектами, они расширяют их возможности:

- элементы хранятся в упорядоченном виде;
- существует специальное свойство `length`, отражающее количество элементов;
- доступны встроенные методы для работы с коллекциями (`push`, `pop`, `map`, `filter`, и т.д.).

Именно эти дополнительные возможности делают массивы удобным инструментом для работы с упорядоченными наборами данных.

Чтобы лучше понять идею, можно представить массив как обычный объект со специальной структурой. Например, упрощённая реализация массива может выглядеть так:
...
Разумеется, реальные массивы в JavaScript реализованы гораздо сложнее и оптимизированы на уровне движка. _Этот пример нужен не для практического использования, а для понимания концепции_.

## Теперь вы знаете ...

1. Что такое объекты в JavaScript и почему они являются универсальным способом описания сущностей реального мира через свойства и действия.
2. Как создавать объекты с помощью литерала `{}` и почему этот способ используется чаще всего.
3. Как работать со свойствами объектов: получать их, добавлять, изменять и удалять.
4. В чём разница между точечной и скобочной нотацией и в каких ситуациях скобочная нотация является необходимой.
5. Что такое вычисляемые свойства и как формировать имена свойств динамически во время выполнения программы.
6. Как использовать сокращённую запись свойств и методов для более чистого и читаемого кода.
7. Что такое методы объектов и чем они отличаются от обычных функций.
8. Почему объекты (и массивы) являются _ссылочным типом данных_ и что означает передача по ссылке.
9. Какие проблемы могут возникать при копировании объектов и почему простое присваивание не создаёт копию.
10. Что такое деструктуризация объектов, зачем она нужна и как извлекать свойства, переименовывать их и задавать значения по умолчанию.
11. Как использовать rest- и spread-операторы при работе с объектами.
12. Чем `null` отличается от `undefined` и в каких случаях используется `null`.
13. Что такое коллекции `Map` и `Set`, чем они отличаются от обычных объектов и массивов и когда их стоит применять.
14. Что в JavaScript существуют встроенные объекты (`Math`, `Date` и другие), которые предоставляют готовую функциональность.
15. Почему у примитивов есть методы и как работают временные объекты-обёртки (`String`, `Number`, `Boolean`).
16. Почему массивы - это на самом деле объекты с числовыми ключами и дополнительным поведением.
17. Что понимание объектов - это фундамент для дальнейшего изучения JavaScript, включая функции, классы, асинхронность и работу с фреймворками.
